# The Salience Model — Full Guide

> New here? Start with the [README](README.md) — it's the short, friendly version. This page is the full manual.
...
3. **Fill in `CORE.md`.** Copy `CORE.template.md` to `CORE.md` and write it honestly. This is the one file that has to be *yours* — it's what the AI reads every session to know who it's working with. Keep it under a page.
   ...
   **Easier option:** open Claude Code in the folder and run `/setup` — it interviews you, one question at a time, and drafts `CORE.md` for your approval.
...
Three commands cover almost everything (plus `/setup`, which you typically run once). In Claude Code they're slash commands; on the web, just describe the mode.

- **`/setup`** — first-run interview. Asks you plain-language questions and drafts your `CORE.md` for approval. Run it again later to revise.
...
| `.claude/commands/*.md` | The `/setup`, `/capture`, `/review`, `/brainstorm` commands. | Rarely. |
